...
python example/manage.py test --with-xunit --with-xcover --cover-package=class_registry
TEST_EXIT=$?
rm -rf jenkins_reports
mkdir jenkins_reports
pep8 class_registry > jenkins_reports/pep8.report
PEP8_EXIT=$?
pyflakes class_registry > jenkins_reports/pyflakes.report
PYFLAKES_EXIT=$?
